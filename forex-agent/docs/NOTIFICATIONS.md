...
|---|---|---|---|
| Agent | `agent` | always | **Primary.** Delivery = presence in the delivery journal; the SSE stream carries it to the agent. `send()` verifies the event is journaled and re-journals it if missing (idempotent: `INSERT OR IGNORE` on `event_id`). **Proactive delivery** is done by the notification bridge (`agent/events/bridge.py`, managed `agent_bridge` service): it subscribes to the SSE stream and pushes each event to the host-provided sink — see `docs/EVENTS.md` ("The notification bridge"). |
